...
optimal_cutpoint <- function(data, ..., choice, metric = c("Accuracy", "Kappa", "Sensitivity", "Specificity", "Balanced"), names = NULL){
  ...
  include <- array(NA, dim = c(nrow(ptv.matrix), ncol(ptv.matrix), max - min - 1))
  for(i in min:(max - 1)){
  ...
  for(i in seq_len(ncol(ptv.matrix))){
    for(j in seq_len(max - min - 1)){
      ...
      if(metric == "Accuracy")
        ...
        perform[j,i] <- caret::confusionMatrix(t, positive = "1")$byClass[11]
    }
  }
  party.cutpoints <- apply(perform, 2, which.max) - min
  global.cutpoint <- which.max(rowMeans(perform)) - min
